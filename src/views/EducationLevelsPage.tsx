...
import { ChevronRight, GraduationCap, Search, Frown, ArrowRight } from "lucide-react";
...
// Curated modern theme classes tailored specifically for each qualification
...
export function EducationLevelsPage({ initialEducations = [] }: EducationLevelsPageProps) {
  ...
  useEffect(() => {
    ...
    const getEducations = async () => {
      try {
        ...
        console.error("Failed to fetch educations for list page:", err);
      }
    };
    getEducations();
  }, [initialEducations]);
  ...
  const filteredLevels = useMemo(() => {
    if (!searchQuery.trim()) return mergedEducations;
    return mergedEducations.filter((edu) =>
      edu.label.toLowerCase().includes(searchQuery.toLowerCase()) ||
      edu.description.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [mergedEducations, searchQuery]);
        ...
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 relative z-10">
          <FadeIn>
            <nav className="flex items-center gap-2 text-sm text-slate-custom mb-4">
              ...
              <span className="text-white font-medium">Education Levels</span>
            </nav>
          </FadeIn>
          <FadeIn delay={0.1}>
            ...
            <div className="mt-6 max-w-lg">
              ...
              {searchQuery && (
                <p className="text-[11px] text-brand font-bold mt-3 animate-fadeIn">
                  Showing {filteredLevels.length} matches for &ldquo;{searchQuery}&rdquo;
                </p>
              )}
            </div>
          </FadeIn>
      ...
      <section className="py-10 md:py-16">
        <div className="max-w-[1440px] mx-auto px-4 md:px-8">
          ...
          {filteredLevels.length > 0 ? (
            <StaggerContainer
              staggerDelay={0.05}
              className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 mt-6"
            >
              {filteredLevels.map((edu) => {
                const theme = cardThemes[edu.slug] || {
                  color: "text-slate-800",
                  iconBg: "bg-brand/10",
                  iconColor: "text-brand",
                  hoverIconBg: "group-hover:bg-brand",
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between mb-4 relative z-10">
                        <div className={`w-12 h-12 rounded-2xl ${theme.iconBg} ${theme.hoverIconBg} flex items-center justify-center group-hover:rotate-6 group-hover:scale-105 transition-all duration-300`}>
                          <GraduationCap className={`w-6 h-6 ${theme.iconColor} group-hover:text-white transition-colors duration-300`} />
                        </div>
                      ...
                      <h3 className={`text-base font-bold mb-1.5 transition-colors relative z-10 ${theme.color}`}>
                        <HighlightMatch text={edu.label} query={searchQuery} />
                      </h3>
                      <p className="text-[12px] font-medium text-slate-500 leading-relaxed mb-4 relative z-10 line-clamp-2">
                        <HighlightMatch text={edu.description} query={searchQuery} />
                      </p>
            ...
            /* Premium Illustrated Qualification Empty State */
            <div className="bg-white/70 backdrop-blur-xl border border-white/60 shadow-card rounded-2xl py-16 px-4 text-center mt-6 animate-fadeIn max-w-xl mx-auto">
              ...
              <p className="text-[11.5px] text-slate-500 max-w-xs mx-auto mb-4 leading-relaxed font-medium">
                We couldn&rsquo;t find any education levels matching &ldquo;{searchQuery}&rdquo;. Try another qualification (e.g. 10th, B.Tech, Graduate).
              </p>
              <button
                onClick={() => setSearchQuery("")}
