...
export function BlogDetailPage({ initialBlog, initialBlogs, initialSettings, slug }: BlogDetailPageProps) {
  const settings = initialSettings;
  const router = useRouter();
  const [blog] = useState<BlogItem | null>(initialBlog);
  const [blogs] = useState<BlogItem[]>(initialBlogs);
              ...
              <div className="lg:sticky lg:top-24 space-y-5">
                ...
                <FadeIn direction="left" delay={0.15}>
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden p-4 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-emerald-500 font-extrabold px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 uppercase tracking-wider">
                      Live Stats
                    </span>
                  </div>
                </FadeIn>
