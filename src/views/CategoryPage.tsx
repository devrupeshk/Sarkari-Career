...
import { Briefcase, ChevronRight, ChevronLeft, Search, ArrowRight, Frown, GraduationCap, Zap, Bell, Building2, Landmark, BookOpen, Train, Shield, Stethoscope, FileText, Globe } from "lucide-react";
...
// Curated modern theme config for every single category
...
> = {
  "current-job": {
    ...
    badgeBg: "bg-brand/10 text-brand border-brand/20 shadow-[0_0_10px_rgba(220,38,38,0.1)]",
    badgeActiveBg: "bg-brand-light text-brand",
    activeSidebarBg: "bg-gradient-to-r from-brand to-red-500 text-white shadow-lg shadow-brand/20 scale-[1.02] border-transparent",
    rowHoverBorder: "hover:border-l-brand",
    statusDot: "bg-brand shadow-sm shadow-brand/50",
    arrowColor: "text-brand",
    heroTextGradient: "from-brand to-red-400",
  },
  bank: {
...
export function CategoryPage({ slug, jobs, total }: CategoryPageProps) {
  ...
  const theme = categoryThemes[slug] || categoryThemes["current-job"];
  const ActiveIcon = iconMap[meta.icon] || Briefcase;
  ...
  const filteredJobs = categoryJobs;
  ...
  return (
    <div className="min-h-screen pt-14 bg-off-white overflow-hidden relative">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6 mb-6">
            ...
            <FadeIn className="shrink-0" delay={0.1}>
              <div className={`w-16 h-16 rounded-2xl border flex items-center justify-center relative group transition-all duration-300 hover:rotate-6 ${theme.iconBg}`}>
                <ActiveIcon className={`w-8 h-8 relative z-10 ${theme.iconColor}`} />
              </div>
            </FadeIn>
          ...
          <FadeIn delay={0.3}>
            <div className="max-w-lg">
              ...
              >
                ...
                <input
                  type="text"
                  placeholder={`Search in ${meta.title} jobs...`}
                  readOnly
      ...
      <section className="py-10 bg-off-white">
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 relative z-10">
          <SectionHeader label="📋 LISTINGS" heading={`${meta.title} Vacancies`} />
          ...
          <div className="flex flex-col lg:flex-row gap-6 mt-4">
            ...
            <div className="flex-1 lg:w-[70%]">
                  ...
                  {filteredJobs.length > 0 ? (
                    <StaggerContainer staggerDelay={0.03}>
                      {filteredJobs.map((job, i) => {
                        ...
                        // API jobs use 'slug' directly; no need to generate one
                        const jobSlug = (job as ApiJobItem).slug || generateSlug(job.title);
                        ...
                        return (
                          <StaggerItem key={i}>
                            ...
                            <div className="md:hidden px-4 py-3.5 border-b border-slate-100/60 hover:bg-white transition-all duration-300 group">
                              <Link href={`/post/${jobSlug}`} className="block">
                    ...
                    <div className="py-16 px-4 text-center animate-fadeIn">
                      ...
                      <p className="text-[11.5px] text-slate-500 max-w-xs mx-auto mb-4 leading-relaxed font-medium">
                        There are currently no active job vacancies in the {meta.title} category. Please check back later for updates.
                      </p>
                    </div>
                  )}
                </div>
              ...
              {/* Pagination (only show if listings exceed 10 or for aesthetic filler) */}
                ...
                <div className="bg-white/70 backdrop-blur-xl border border-white/60 shadow-card rounded-2xl p-4.5 relative overflow-hidden group">
                  ...
                  <ul className="space-y-1.5 relative z-10">
                    {sidebarCategories.map((cat) => {
                      ...
                      const catTheme = categoryThemes[cat.slug] || categoryThemes["current-job"];
                      return (
                        <li key={cat.slug}>
                          <Link
                            href={`/${cat.slug}`}
                            className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-[12.5px] font-bold transition-all duration-300 border ${isActive
                              ? `${catTheme.activeSidebarBg} text-white`
      ...
      {/* Category-Specific Command Search Dialog */}
      <CommandDialog open={isSearchOpen} onOpenChange={setIsSearchOpen}>
        <CommandInput placeholder={`Search in ${meta.title} jobs...`} />
        <CommandList className="max-h-[400px] overflow-y-auto">
          <CommandEmpty>No matches found.</CommandEmpty>
          <CommandGroup heading={`${meta.title} Jobs`}>
            {categoryJobs.map((item) => {
              ...
              return (
                ...
                >
                  <Briefcase className="mr-2.5 h-4 w-4 text-brand shrink-0" />
