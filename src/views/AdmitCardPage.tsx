...
import { FileText, ChevronRight, ChevronLeft, Search, ArrowRight, Frown, GraduationCap, Zap, Bell, Building2, Landmark, BookOpen, Train, Shield, Briefcase, Stethoscope, Globe } from "lucide-react";
import {
  CommandDialog,
  CommandInput,
  CommandList,
...
export function AdmitCardPage({ jobs }: AdmitCardPageProps) {
  const router = useRouter();
  const settings = useSettings();
  const [isSearchOpen, setIsSearchOpen] = useState(false);
  const searchQuery = ""; // Read-only on page
  ...
  return (
    <div className="min-h-screen pt-14 bg-off-white overflow-hidden relative">
      ...
      <div className="absolute top-[40%] left-0 w-[500px] h-[500px] bg-sky-500/5 rounded-full blur-[120px] pointer-events-none -translate-x-1/2" />
      ...
      <section className="relative bg-navy pt-8 pb-12 md:pt-10 md:pb-16 overflow-hidden">
        {/* Dynamic backdrop glows matching sky-blue/indigo theme */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-sky-500/20 via-navy to-navy" />
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-sky-500/10 rounded-full blur-[120px] pointer-events-none -translate-y-1/2 translate-x-1/3" />
        ...
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 relative z-10">
          <FadeIn>
            <nav className="flex items-center gap-2 text-sm text-slate-custom mb-5">
              ...
              <span className="text-white font-medium">Admit Card</span>
            </nav>
          </FadeIn>

          <div className="flex flex-col md:flex-row md:items-center gap-6 mb-6">
            <FadeIn className="shrink-0" delay={0.1}>
              <div className="w-16 h-16 rounded-2xl border border-sky-500/20 bg-sky-500/10 flex items-center justify-center relative group transition-all duration-300 hover:rotate-6 shadow-[0_0_30px_rgba(14,165,233,0.15)]">
                <FileText className="w-8 h-8 relative z-10 text-sky-400" />
              </div>
            </FadeIn>
          ...
          <FadeIn delay={0.25}>
            <p className="text-[13.5px] md:text-[15px] text-slate-custom max-w-2xl leading-relaxed font-medium mb-6">
              Browse and download the latest verified admit cards, hall tickets, call letters, and check exam dates for central and state government recruitment exams across India.
            </p>
          </FadeIn>
          ...
          <FadeIn delay={0.3}>
            <div className="max-w-lg">
              ...
              >
                ...
                <input
                  type="text"
                  placeholder="Search in Admit Cards..."
                  readOnly
      ...
      <section className="py-10 bg-off-white">
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 relative z-10">
          <SectionHeader label="📋 LISTINGS" heading="Latest Admit Cards" />
                  ...
                  {filteredJobs.length > 0 ? (
                    <StaggerContainer staggerDelay={0.03}>
                      {filteredJobs.map((job, i) => {
                        ...
                        const jobSlug = job.slug || generateSlug(job.title);
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2.5 relative z-10 truncate">
                                ...
                                <span className="text-[12px] font-medium text-slate-700 group-hover:text-brand transition-all duration-300 truncate flex items-center gap-1">
                                  <span>{job.title}</span>
                                  <ArrowRight className="w-3.5 h-3.5 text-sky-500 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 shrink-0" />
                                </span>
                              </div>
                    ...
                    <div className="py-16 px-4 text-center animate-fadeIn">
                      ...
                      <p className="text-[11.5px] text-slate-500 max-w-xs mx-auto mb-4 leading-relaxed font-medium">
                        There are currently no active admit cards available. Please check back later for updates.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <CommandDialog open={isSearchOpen} onOpenChange={setIsSearchOpen}>
        <CommandInput placeholder="Search in Admit Cards..." />
        <CommandList className="max-h-[400px] overflow-y-auto">
          <CommandEmpty>No matches found.</CommandEmpty>
          <CommandGroup heading="Admit Cards / Hall Tickets">
            {jobs.map((item) => {
              ...
              return (
                ...
                >
                  <FileText className="mr-2.5 h-4 w-4 text-sky-500 shrink-0" />
