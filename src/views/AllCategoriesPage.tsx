...
import { ChevronRight, Search, Frown, ArrowRight } from "lucide-react";
...
// Curated modern theme classes tailored specifically for each category card
const cardThemes: Record<string, {
  ...
  "current-job": {
    color: "text-brand group-hover:text-red-500",
    iconBg: "bg-red-50",
...
export function AllCategoriesPage() {
  ...
  const filteredCategories = useMemo(() => {
    if (!searchQuery.trim()) return allCategoryDefs;
    return allCategoryDefs.filter(
      (cat) =>
        cat.label.toLowerCase().includes(searchQuery.toLowerCase()) ||
        cat.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (cat.subcategories && cat.subcategories.some(sub => sub.toLowerCase().includes(searchQuery.toLowerCase())))
    );
  }, [searchQuery]);
            ...
            <div className="mt-6 max-w-lg">
              ...
              {searchQuery && (
                <p className="text-[11px] text-brand font-bold mt-3 animate-fadeIn">
                  Showing {filteredCategories.length} matches for &ldquo;{searchQuery}&rdquo;
                </p>
              )}
            </div>
          </FadeIn>
          ...
          {filteredCategories.length > 0 ? (
            ...
            >
              {filteredCategories.map((cat) => {
                const theme = cardThemes[cat.slug] || {
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between mb-4 relative z-10">
                        <div className={`w-12 h-12 rounded-2xl ${theme.iconBg} ${theme.hoverIconBg} flex items-center justify-center group-hover:rotate-6 group-hover:scale-105 transition-all duration-300`}>
                          <IconComponent className={`w-6 h-6 ${theme.iconColor} group-hover:text-white transition-colors duration-300`} />
                        </div>
                      ...
                      <p className="text-[12px] font-medium text-slate-500 leading-relaxed mb-4 relative z-10 line-clamp-2">
                        <HighlightMatch text={cat.description} query={searchQuery} />
                      </p>

                      {/* Subcategory Pills */}
                      {cat.subcategories && (
                        <div className="flex flex-wrap gap-1 mb-5 relative z-10">
                          {cat.subcategories.slice(0, 4).map((sub) => (
                            <span
                              key={sub}
                              className="text-[9.5px] font-bold bg-slate-50 text-slate-500 border border-slate-100 px-2 py-0.5 rounded-md"
                            >
                              <HighlightMatch text={sub} query={searchQuery} />
                            </span>
                          ))}
                        </div>
                      )}
            ...
            /* Premium Illustrated Category Empty State */
            <div className="bg-white/70 backdrop-blur-xl border border-white/60 shadow-card rounded-2xl py-16 px-4 text-center mt-6 animate-fadeIn max-w-xl mx-auto">
              ...
              <p className="text-[11.5px] text-slate-500 max-w-xs mx-auto mb-4 leading-relaxed font-medium">
                We couldn&rsquo;t find any job categories matching &ldquo;{searchQuery}&rdquo;. Try searching for keyword (e.g. Bank, Railway, Civil).
              </p>
              <button
                onClick={() => setSearchQuery("")}
