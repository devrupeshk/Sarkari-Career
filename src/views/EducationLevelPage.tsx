...
import { GraduationCap, ChevronRight, ArrowLeft, Search, ArrowRight, Frown, Zap, Bell } from "lucide-react";
...
interface EducationLevelPageProps {
  ...
  jobs: ApiJobItem[];
  total?: number;
  initialEducations?: ApiEducationItem[];
}
...
// Highly tailored premium styling variables matching individual qualification themes
...
> = {
  ...
  "10th-pass": {
    ...
    badgeBg: "bg-cyan-500/10 text-cyan-400 border-cyan-500/20 shadow-[0_0_10px_rgba(6,182,212,0.1)]",
    badgeActiveBg: "bg-cyan-50 text-cyan-600",
    activeSidebarBg: "bg-gradient-to-r from-cyan-500 to-cyan-600 text-white shadow-lg shadow-cyan-500/20 scale-[1.02] border-transparent",
    rowHoverBorder: "hover:border-l-cyan-500",
    statusDot: "bg-cyan-500 shadow-sm shadow-cyan-500/50",
    arrowColor: "text-cyan-500",
    heroTextGradient: "from-cyan-400 to-blue-300",
  },
  "12th-pass": {
    color: "text-sky-800",
  ...
  btech: {
    ...
    glowBg: "from-fuchsia-500/20 via-navy to-navy",
    radialGlow: "bg-fuchsia-500/10",
    iconBg: "bg-fuchsia-500/10 border-fuchsia-500/20 shadow-[0_0_30px_rgba(217,70,239,0.15)]",
    iconColor: "text-fuchsia-400",
    badgeBg: "bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/20 shadow-[0_0_10px_rgba(217,70,239,0.1)]",
    badgeActiveBg: "bg-fuchsia-50 text-fuchsia-600",
    activeSidebarBg: "bg-gradient-to-r from-fuchsia-500 to-fuchsia-600 text-white shadow-lg shadow-fuchsia-500/20 scale-[1.02] border-transparent",
    rowHoverBorder: "hover:border-l-fuchsia-500",
    statusDot: "bg-fuchsia-500 shadow-sm shadow-fuchsia-500/50",
    arrowColor: "text-fuchsia-500",
    heroTextGradient: "from-fuchsia-400 to-pink-300",
  },
  bcom: {
    color: "text-pink-800",
  ...
  graduate: {
    ...
    badgeBg: "bg-rose-500/10 text-rose-400 border-rose-500/20 shadow-[0_0_10px_rgba(244,63,94,0.1)]",
    badgeActiveBg: "bg-rose-50 text-rose-600",
    activeSidebarBg: "bg-gradient-to-r from-rose-500 to-rose-600 text-white shadow-lg shadow-rose-500/20 scale-[1.02] border-transparent",
    rowHoverBorder: "hover:border-l-rose-500",
    statusDot: "bg-rose-500 shadow-sm shadow-rose-500/50",
    arrowColor: "text-rose-500",
    heroTextGradient: "from-rose-400 to-orange-300",
  },
  "post-graduate": {
    ...
    badgeBg: "bg-amber-500/10 text-amber-400 border-amber-500/20 shadow-[0_0_10px_rgba(245,158,11,0.1)]",
    badgeActiveBg: "bg-amber-50 text-amber-600",
    activeSidebarBg: "bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-lg shadow-amber-500/20 scale-[1.02] border-transparent",
    rowHoverBorder: "hover:border-l-amber-500",
    statusDot: "bg-amber-500 shadow-sm shadow-amber-500/50",
    arrowColor: "text-amber-500",
    heroTextGradient: "from-amber-400 to-yellow-300",
  },
};

export function EducationLevelPage({ slug, jobs, total, initialEducations = [] }: EducationLevelPageProps) {
  const router = useRouter();
  const settings = useSettings();
  const [isSearchOpen, setIsSearchOpen] = useState(false);
  const searchQuery = "";
  const [apiEducations, setApiEducations] = useState<ApiEducationItem[]>(initialEducations);
  ...
  useEffect(() => {
    ...
    const getEducations = async () => {
      try {
        ...
        setApiEducations(data);
      } catch (err) {
        console.error("Failed to fetch educations for sidebar:", err);
      }
    };
    getEducations();
  }, [initialEducations]);

  const mergedEducationLevels = useMemo(() => {
    return educationLevels.map((staticEdu) => {
      let targetSlugs = [staticEdu.slug];
      if (staticEdu.slug === "graduate") targetSlugs.push("any-graduate");
      if (staticEdu.slug === "btech") targetSlugs.push("btech-be");

      const apiEdu = apiEducations.find((e) =>
        targetSlugs.includes(e.slug.toLowerCase()) ||
        e.name.toLowerCase() === staticEdu.label.toLowerCase()
      );

      return {
        ...staticEdu,
        count: apiEdu ? String(apiEdu.jobCount) : "0",
      };
    });
  }, [apiEducations]);

  const level = mergedEducationLevels.find((e) => e.slug === slug);
  ...
  // Active theme mapping with bulletproof fallback to 10th-pass styles
  const theme = pageThemes[slug] || pageThemes["10th-pass"];
  ...
  return (
    <div className="min-h-screen pt-14 bg-off-white overflow-hidden relative">
        ...
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 relative z-10">
          <FadeIn>
            <nav className="flex items-center gap-2 text-sm text-slate-custom mb-5">
              ...
              <Link href="/education-levels" className="hover:text-white transition-colors">
                Education Levels
              </Link>
              <ChevronRight className="w-3.5 h-3.5" />
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6 mb-6">
            ...
            <FadeIn className="shrink-0" delay={0.1}>
              <div className={`w-16 h-16 rounded-2xl border flex items-center justify-center relative group transition-all duration-300 hover:rotate-6 ${theme.iconBg}`}>
                <GraduationCap className={`w-8 h-8 relative z-10 ${theme.iconColor}`} />
              </div>
            </FadeIn>
          ...
          <FadeIn delay={0.3}>
            <div className="max-w-lg">
              ...
              >
                ...
                <input
                  type="text"
                  placeholder={`Search in ${level.label} jobs...`}
                  readOnly
                  ...
                  {filteredJobs.length > 0 ? (
                    ...
                    <div className="py-16 px-4 text-center animate-fadeIn">
                      ...
                      <p className="text-[11.5px] text-slate-500 max-w-xs mx-auto mb-4 leading-relaxed font-medium">
                        There are currently no active job vacancies for the {level.label} qualification level. Please check back later for updates.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-white/70 backdrop-blur-xl border border-white/60 shadow-card rounded-2xl p-4.5 relative overflow-hidden group">
                  ...
                  <div className="flex items-center justify-between mb-4 relative z-10">
                    <h4 className="text-[13px] font-bold text-slate-800 flex items-center gap-1.5 uppercase tracking-wide">
                      <GraduationCap className="w-4 h-4 text-brand" /> Education Levels
                    </h4>
                    <Link
                  ...
                  <ul className="space-y-1.5 relative z-10">
                    {mergedEducationLevels.map((edu) => {
                      const isActive = slug === edu.slug;
                      const eduTheme = pageThemes[edu.slug] || pageThemes["10th-pass"];
                      return (
                        <li key={edu.slug}>
                          <Link
                            href={`/${edu.slug}`}
                            className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-[12.5px] font-bold transition-all duration-300 border ${isActive
                              ? `${eduTheme.activeSidebarBg} text-white`
                          ...
                          >
                            ...
                            <span
                              className={`text-[9.5px] font-bold px-2 py-0.5 rounded-full ${isActive ? `${eduTheme.badgeActiveBg}` : "bg-slate-100 text-slate-500"
                                }`}
                            >
                              {edu.count}
                            </span>
      ...
      <CommandDialog open={isSearchOpen} onOpenChange={setIsSearchOpen}>
        <CommandInput placeholder={`Search in ${level.label} jobs...`} />
        <CommandList className="max-h-[400px] overflow-y-auto">
          <CommandEmpty>No matches found.</CommandEmpty>
          <CommandGroup heading={`${level.label} Jobs`}>
            {jobs.map((item) => {
              ...
              return (
                ...
                >
                  <GraduationCap className={`mr-2.5 h-4 w-4 shrink-0 ${theme.iconColor}`} />
